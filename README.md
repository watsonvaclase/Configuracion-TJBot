...
### Clonar microSD card on MacOS
Conecte su tarjeta microSD con Raspbian a su MacBook.
La primera línea de código le permite saber cuál es el número asignado a su tarjeta microSD. Lo verás en la lista.
La segunda línea de código creó una imagen de su tarjeta microSD en su escritorio

    diskutil list
    sudo dd if=/dev/<disk number, e.g. disk4> of=~/Desktop/tjbotcz.img conv=sparse bs=1m
    
Crear la imagen toma un tiempo. Si desea ver el estado, simplemente presione CTRL + T y el progreso se mostrará en el terminal. La imagen creada tendrá el mismo tamaño que la capacidad de la tarjeta microSD (e.g. 8GB or 16GB).

### Cómo editar archivos de forma remota en TJBot directamente desde VS Code
Para poder editar archivos de forma remota, debe tener instalada la versión remota de Código VS en su Visual Code Studio y también tener rmate instalado en su TJBOTCZ (Raspberry Pi).

Instalación de rmate (Raspberry Pi): 
En el terminal abierto de Raspberry pi (nebo de forma remota a través de SSH), ejecute estos comandos:
...
Luego, en la ventana de Terminal de VS Code abra y conéctese a Raspberry Pi:

```
ssh -R 52698:localhost:52698 pi@<ip adresa R-pi>
```

En la ventana de la Terminal, navegue hasta el archivo que desea editar y ejecutar:

```
rmate <název souboru>
```

El archivo se abrirá en VS Code y podrá editarlo.

Sugerencia: si no desea ingresar * ssh -R 52698: localhost: 52698 * * pi @ <ip adresa R-pi> * cada vez, puede crear un alias. En MacOS, cree el archivo .bash_profile debajo de su carpeta de nombre de usuario (si aún no existe) y escriba en el archivo algo como:

```
alias tjpi='ssh -R 52698:localhost:52698 pi@10.0.0.29'
```
En el caso anterior, solo se necesita escribir * tjpi * en el Código VS y está conectado. Por supuesto, la dirección IP debe cambiarse cada vez que Raspberry Pi obtenga una nueva.

### Cómo configurar el botón IP en TJBot
Esta función se activa automáticamente con el script run-me-first. En ese caso, no es necesario realizar la siguiente configuración.

Una vez conectado a Internet a través de un cable Ethernet o a través de WiFi conocido, puede obtener información sobre la dirección IP de TJBotCZ presionando el botón. Debe tener el botón instalado como se describe en el manual ["build"] (https://github.com/tjbotcz/manuals/tree/master/en/build). Opcionalmente, si también tiene instalada la pantalla LCD (consulte ["compilación"] (https://github.com/tjbotcz/manuals/tree/master/en/build)), la dirección IP también aparece en la pantalla.

Instale espeak (un programa de texto a voz fuera de línea):
```
sudo apt-get install espeak
```

Añada la siguiente línea al final del archivo .bashrc (sudo nano ~ / .bashrc):

```
sudo node ~/Desktop/tjbotcz_lite/ipButton.js
```
Cada vez que se reinicia TJBot, él verificará la dirección IP y la dirá presionando el botón.
